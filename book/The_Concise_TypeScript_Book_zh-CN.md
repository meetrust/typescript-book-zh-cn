...
Promise 是一种处理异步操作及其结果的方法，使用 `.then()` 和 `.catch()` 等方法来处理成功和错误条件。
...
要为 @types 包的类型定义做出贡献，请向 [https://github.com/DefinitelyTyped/DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped) 提交 PR 请求。
...
如果您想在项目中使用 ES 模块，可以在 package.json 文件中将 type 属性设置为"module"。这指示 Node.js 将项目视为 ES 模块项目。
